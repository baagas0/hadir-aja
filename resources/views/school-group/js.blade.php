@php
    ...
    $route = [
        'detail' => route('school-group.data.detail'),
        'update' => route('school-group.update'),
        'store' => route('school-group.store'),
        'delete' => route('school-group.delete'),
        'bulk-delete' => route('school-group.bulk.delete'),
    ];
@endphp
<script>
    "use strict";
    ...
    var KTDatatablesServerSide = function () {
        ...
        var initDatatable = function () {
            dt = $("#table_{{ $_id }}").DataTable({
                ...
                ajax: {
                    url: "{{ route('school-group.data') }}",
                },
                columns: [
                    { data: 'id' },
                    { data: 'group_code' },
                    { data: 'group_name' },
                    { data: 'school_shift.shift_name' },
                    { data: 'daily_presence_service.name' },
                    { data: 'is_can_create_presence' },
                    { data: null },
                ],
                columnDefs: [
                    {
                    ...
                    },
                    {
                        ...
                        render: function (data) {
                            return `
                                <span class="ms-2 badge badge-light-${data ? 'success' : 'danger'}" data-toggle="tooltip" data-placement="top" title="Pengguna dalam grup ini dapat membuat presensi kelas jika dizinkan.">${data ? 'Ya, dizinkan' : 'Tidak Diizinkan'}</span>
                            `;
                        }
                    },
                    {
