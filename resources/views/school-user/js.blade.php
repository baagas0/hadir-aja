@php
    ...
    $route = [
        'detail' => route('school-users.data.detail'),
        'update' => route('school-users.update'),
        'store' => route('school-users.store'),
        'delete' => route('school-users.delete'),
        'bulk-delete' => route('school-users.bulk.delete'),
    ];
@endphp
<script>
    "use strict";
    ...
    var KTDatatablesServerSide = function () {
        ...
        var initDatatable = function () {
            dt = $("#table_{{ $_id }}").DataTable({
                ...
                ajax: {
                    url: "{{ route('school-users.data') }}",
                },
                columns: [
                    { data: 'id' },
                    { data: 'student_number' },
                    { data: 'student_name' },
                    { data: 'school_group.group_name' },
                    { data: 'school_position.position_name' },
                    { data: 'gender' },
                    { data: 'email' },
                    { data: 'phone_number' },
                    { data: 'birth_date' },
                    { data: 'is_all_location_presence' },
                    { data: null },
                ],
                columnDefs: [
                    {
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            if (data) return `<span class="badge badge-warning">Semua Lokasi</span>`;
                            else return row.school_location.title ?? '-'
                        }
                    },
                    {
                        targets: -1,
        ...
        var modalShow = function () {
            ...
            // LISTEN
            $('#is_all_location_presence').on('change', () => {
                const el = $('#is_all_location_presence')
                if (el.is(":checked")) {
                    el.val(1)
                    $('#school_location_id_element').css('visibility', 'collapse')
                } else {
                    el.val(0)
                    $('#school_location_id_element').css('visibility', 'visible')
                }
            })
        }
